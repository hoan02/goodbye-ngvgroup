...
export default function AdminPage() {
  ...
  const handleCreateProfile = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/profiles/list', {
        ...
        body: JSON.stringify({ name: newProfileName, slug }),
      });
      if (response.ok) {
        ...
        toast.error('Failed to create profile. Slug might exist.');
      }
    } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/profiles/update?id=${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
      ...
      toast.error('Error deleting profile', { id: toastId });
      console.error(error);
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {profiles.map(profile => (
            <div key={profile.id} className="bg-card border border-border rounded-2xl p-6 flex flex-col h-full shadow-sm">
