...
import { Save, ArrowLeft, Plus, Trash2, GripVertical, User, Layout } from 'lucide-react';
...
export default function ProfileEditor({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchProfileConfig = async () => {
    try {
      const data = await getProfileById(id) as ProfileData;
      // Ensure each section has a temporary ID for Reorder tracking if it doesn't have one
      if (data && data.sections) {
        data.sections = data.sections.map((s: Section, i: number) => ({
          ...s,
  ...
  if (loading) return <div className="p-8 text-center animate-pulse min-h-screen bg-background text-foreground flex justify-center items-center uppercase tracking-widest text-sm font-light">Loading Profile Config...</div>;
  if (!profile) return <div className="p-8 text-center text-destructive min-h-screen bg-background flex flex-col justify-center items-center font-bold text-2xl">Profile not found.</div>;
                              ...
                              <div className="flex items-center justify-between gap-4">
                                <div className="flex items-center gap-3 flex-grow">
                                  ...
                                  <input 
                                    ...
                                    className="font-bold bg-transparent border-none focus:ring-0 p-0 text-xl w-full placeholder:opacity-30"
                                  />
                                </div>
                                <button 
