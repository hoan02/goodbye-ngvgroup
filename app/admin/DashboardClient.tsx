...
import { createProfile, deleteProfile } from '@/app/actions/profile';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export type ProfileSummary = { _id: string; slug: string; name: string; role?: string; thumbUrl?: string };

export default function DashboardClient({ initialProfiles }: { initialProfiles: ProfileSummary[] }) {
          ...
          {profiles.map(profile => (
            <div key={profile._id} className="bg-card border border-border rounded-2xl p-6 flex flex-col h-full shadow-sm">
              <div className="flex items-center gap-4 mb-6 text-left">
                {profile.thumbUrl ? (
                   <div 
                     className="w-12 h-12 rounded-xl flex items-center justify-center min-w-[3rem] bg-cover bg-center border border-border"
